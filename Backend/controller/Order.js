...
exports.fetchOrdersByUser = async (req, res) => {
  ...
  exports.fetchOrderById = async (req, res) => {
    ...
    try {
      ...
      // Populate product details in items if possible
      if (order.items && order.items.length > 0) {
        const productIds = order.items.map(item => item.product && item.product.id ? item.product.id : item.product);
        const products = await Product.find({ _id: { $in: productIds } });
        order.items = order.items.map(item => {
          const prod = products.find(p => p._id.toString() === (item.product?.id || item.product?.toString()));
